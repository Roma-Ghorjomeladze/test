...
Gutschine.getInitialProps = async function(ctx) {
  ...
  const content = await import(`../../data/andrea-schuppli/${slug}.md`)
  const config = await import(`../../data/andrea-schuppli/config.json`)
  const data = matter(content.default)

  return {
    markdownFile: {
      fileRelativePath: `data/andrea-schuppli/${slug}.md`,
